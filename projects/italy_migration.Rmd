---
title: "Italy Population & Migration Analysis"
author: "CVAnstey"
date: "2026-06-01"
...
```{r map-population}
ggplot(italy_prov_map %>% filter(time_period == 2024)) +
  geom_sf(aes(fill = population), color = NA) +
  scale_fill_viridis_c(labels = comma, na.value = "white") +
  theme_void() +
  labs(title = "Italy: Population by Province (2024)", fill = "Population")
```
...
ggplot(top_origins, aes(x = stock_2024, y = reorder(country, stock_2024))) +
...
p_lollipop <- ggplot(movers, aes(x = pct_change, y = territory, color = direction)) +
  geom_segment(aes(x = 0, xend = pct_change, yend = territory), linewidth = 0.8) +
  geom_point(size = 3.5) +
...
```{r patchwork-spatial, fig.width=14, fig.height=8}
(p_change | p_cluster) +
  plot_annotation(
...
```{r patchwork-trajectories, fig.width=12, fig.height=12}
...
```{r patchwork-facet, fig.width=16, fig.height=6}
p_facet +
  plot_annotation(
